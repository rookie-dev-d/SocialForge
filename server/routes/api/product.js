  ...
  Product.findOne({ sku }, (err, existingProduct) => {
    if (err) {
      return res.status(400).json({
        error: '1Your request could not be processed. Please try again.'+err
      });
    }
    ...
    product.save((err, data) => {
      if (err) {
        return res.status(400).json({
          error: '2Your request could not be processed. Please try again.'+err
        });
      }
...
router.get('/list/category/:slug', (req, res) => {
  ...
  Category.findOne({ slug: slug }, 'products -_id')
    ...
    .exec((err, data) => {
      if (err) {
        return res.status(400).json({
          error: '3Your request could not be processed. Please try again.'+err
        });
      }
