...
export const fetchOrders = (order) => {
  return async (dispatch, getState) => {
    try {
      ...
      console.log("real "+response.data.useroradman);

      if (response.data.orders) {
        dispatch({
          type: FETCH_ORDERS,
          payload: response.data.orders,
          useroradman:response.data.useroradman
        });
      }
    } catch (error) {
...
export const completeOrderItem = itemId => {
  console.log('this action is called');
  return async (dispatch, getState) => {
    try {
      const order = getState().order.order;

      const response = await axios.patch(`/api/order/complete/${itemId}`, {
        cartId: order.cartId
      });
